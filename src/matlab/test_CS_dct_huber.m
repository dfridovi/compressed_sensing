...
for i = 1:length(ALPHA)
   ...
   reconstruction = assembleBlocks(reconstructed_blocks, BLOCK_SIZE, ...
    ...
    filename = sprintf('../../reconstructions/matlab figures/cs_dct_size%dx%d_rho%1dp%1d_alpha%1dp%1d_overlap%1dp%1d_oversample%1dp%1d.mat', ...
    ...
    filename = sprintf('../../reconstructions/matlab figures/cs_dct_size%dx%d_rho%1dp%1d_alpha%1dp%1d_overlap%1dp%1d_oversample%1dp%1d.png', ...
        IMAGE_SIZE(1), IMAGE_SIZE(2), floor(RHO), 10*mod(RHO, 1), floor(alpha), 10*mod(alpha, 1), ...
        floor(OVERLAP_PERCENT), 10*mod(OVERLAP_PERCENT, 1), ...
        floor(BASIS_OVERSAMPLING), 10*mod(BASIS_OVERSAMPLING, 1));
    saveas(gcf, filename);
